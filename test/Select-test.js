...
describe('Select', () => {
	...
	var findAndFocusInputControl = () => {
		...
		var searchInstance = instance.find('input')[0];
		if (searchInstance) {
			return searchInstance.simulate('focus');
		}
	};
	...
	describe('with simple options', () => {
		...
		it('should assign the given name', () => {
			var selectInputElement = instance;
			expect(selectInputElement.props().name).to.equal('form-field-name');
		});

		it('should show the options on mouse click', function () {
			instance.find('.Select-control').simulate('click');
			// TestUtils.Simulate.mouseDown(ReactDOM.findDOMNode(instance).querySelector('.Select-control'));
			console.log(instance.find('.Select-option').length);
			console.log(instance.state('isOpen'));
			console.log(instance.find('.Select-control').length);
			expect(instance.find('.Select-option').length).to.equal(3);
		});
	});
